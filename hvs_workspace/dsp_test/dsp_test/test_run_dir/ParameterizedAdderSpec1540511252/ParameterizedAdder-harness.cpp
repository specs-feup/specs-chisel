...
#endif
#include <iostream>
class ParameterizedAdder_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    ParameterizedAdder_api_t(VParameterizedAdder* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_a2)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_a1)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_c)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["ParameterizedAdder.reset"] = 0;
    }
#if VM_TRACE
...
static ParameterizedAdder_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VParameterizedAdder* top = new VParameterizedAdder;
    std::string vcdfile = "test_run_dir/ParameterizedAdderSpec1540511252/ParameterizedAdder.vcd";
...
#endif
    ParameterizedAdder_api_t api(top);
