...
#endif
#include <iostream>
class SimpleDspModule_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    SimpleDspModule_api_t(VSimpleDspModule* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorSData(&(dut->io_y)));
        sim_data.inputs.push_back(new VerilatorSData(&(dut->io_x)));
        sim_data.outputs.push_back(new VerilatorSData(&(dut->io_z)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["SimpleDspModule.reset"] = 0;
    }
#if VM_TRACE
...
static SimpleDspModule_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VSimpleDspModule* top = new VSimpleDspModule;
    std::string vcdfile = "test_run_dir/simple_dsp_fix/SimpleDsp.SimpleDspModuleSpec1620288802/SimpleDspModule.vcd";
