...
class SCIEPipelinedInterface(width : Int, binarypoint : Int) extends Bundle{
  ...
  val rs1 = Input(DspComplex(FixedPoint(width.W, binarypoint.BP), FixedPoint(width.W, binarypoint.BP)))
  val rs2 = Input(UInt(32.W))
  val rd = Output(DspComplex(FixedPoint(width.W, binarypoint.BP), FixedPoint(width.W, binarypoint.BP)))
}
