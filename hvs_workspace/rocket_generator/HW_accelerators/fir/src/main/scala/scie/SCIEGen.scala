...
class SCIEPipelineGenStage extends GenericTesterStage((params, _) => {
  ...
  //type 0 -> unsigned; type 1 -> signed; type 2, 3 -> fixed-point
  fir_type match{

    case "unsigned" | "0" => (new chisel3.stage.ChiselStage).execute(
      Array("--emission-options=disableMemRandomization,disableRegisterRandomization","-X", "verilog"),
    ...
    case "fixed-point" | "2" | "3" => (new chisel3.stage.ChiselStage).execute(
      Array("--emission-options=disableMemRandomization,disableRegisterRandomization","-X", "verilog"),
